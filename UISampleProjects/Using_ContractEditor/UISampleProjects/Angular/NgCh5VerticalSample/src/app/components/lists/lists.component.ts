...
export class ListsComponent implements OnInit {
  ...
  private addSubscriptions(previousItemsCount: number, noItems: number) {
    for (let idx = previousItemsCount; idx < noItems; idx++) {
      ...
      this.selectedSubscriptions[idx] = this.selectedSubscriptions[idx] = CrComLib.subscribeState('b', `ContactList.Contact[${idx}].ContactIsSelected`, (isSelected: any) => {
  ...
  private setListHeight() {
    const excludeSpace = this.windowSize === 'desktop' ? 44 : 22;
